...
function Card(props) {
    ...
    const deleteData = async (e) => {
        ...
        return auth.currentUser.getIdToken().then(async token => {
            console.log(token)
            await fetch(`https://us-central1-cv-builder-327dd.cloudfunctions.net/api/personal/${props.information.id}`,
                {
                    method: "DELETE",
                    headers: {
    ...
    function nav() {
        setIsClicked(true);
        navigate("EditPersonal", { replace: true });
    }
            ...
            <div>
                {isDesktopOrLaptop && <div className="card card-background text-white mb-3" >
                    <div className="card-body">
                        ...
                        <h6 className="card-subtitle mb-2">{props.information.role}</h6>
                        <p className="card-text">{props.information.brief}</p>
                    </div>
                    {isClicked && <Routes>
                        <Route path="EditPersonal" element={<EditPersonal info={props.information} getData={props.getData} />}></Route>
                    </Routes>}
                </div>}
