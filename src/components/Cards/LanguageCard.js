...
function Card(props) {
    ...
    const levelList = ["Beginner", "Good", "Advanced", "Fluent", "Native"]
    function nav() {
        setIsClicked(true);
        navigate("EditLanguages", { replace: true });
    }
        ...
        <div>
            {isDesktopOrLaptop && <div className="card card-background text-white mb-3" >
                <div className="card-body">
                    ...
                    <h6 className="card-subtitle mb-2">{levelList[props.info.level]}</h6>
                </div>
                {isClicked && <Routes>
                    <Route path="EditLanguages" element={<EditLanguage info={props.info} getData={props.getData} />}></Route>
                </Routes>}
            </div>}
