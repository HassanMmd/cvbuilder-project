...
function Card(props) {
    ...
    function nav() {
        setIsClicked(true);
        navigate("EditEducation", { replace: true });
    }
        ...
        <div>
            {isDesktopOrLaptop && <div className="card card-background text-white mb-3" >
                <div className="card-body">
                    ...
                    <h6 className="card-subtitle mb-2">{props.info.school}</h6>
                    <p className="card-text">{props.info.country}</p>
                    <div className="date">
                        <p>{props.info.start_date}     -      {props.info.end_date}</p></div>
                </div>
                {isClicked && <Routes>
                    <Route path="EditEducation" element={<EditEducation info={props.info} getData={props.getData} />}></Route>
                </Routes>}
            </div>}
